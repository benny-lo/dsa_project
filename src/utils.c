...
{
    if (info == NULL) return; 
    if (info->forced != NULL) free(info->forced);
    if (info->appear != NULL) free(info->appear); 
...
void gen_constraint(char ref[], char guess[], char constraint[], help_t * info, int k)
{
        ...
        if (constraint[i] != '+') {
            if (ref_occur[char_index(guess[i])] != 0) {
                ...
                if (!info->option[char_index(guess[i])]) {
                    if (guess_occur_notslash[char_index(guess[i])] > info->occur[char_index(guess[i])]) {
                        info->occur[char_index(guess[i])] = guess_occur_notslash[char_index(guess[i])]; 
                    } 
                } 
            } else {
                constraint[i] = '/';
...
bool compatible(const char * my_str, const help_t * info, size_t k)
{
    ...
    for(size_t i = 0; i < k; i++) {
        ...
        if (info->forced[i] != FOO) {
            if (info->forced[i] != my_str[i]) return false; 
        } else {
            if (!info->appear[i * ALPHABET_SIZE + char_index(my_str[i])]) return false; 
        }
    }

    for(int i = 0; i < ALPHABET_SIZE; i++) {
        if (info->option[i]) {
            if (info->occur[i] != str_occur[i]) return false; 
        } else {
            if (info->occur[i] > str_occur[i]) return false; 
        }
    }
