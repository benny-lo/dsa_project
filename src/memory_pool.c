...
void memory_pool_copy(memory_pool_t *pool, const char *request,
                      size_t request_size, size_t *copied);
...
memory_pool_t *memory_pool_create(size_t initial_capacity) {
  ...
  if (pool == NULL) {
    fprintf(stderr,
            "failed allocation of memory_pool_t in memory_pool_create\n");
    return NULL;
  }
...
bool memory_pool_request(memory_pool_t *pool, const char *request,
  ...
  if (request_size > real_capacity - real_size) {
    if (pool->capacity > SIZE_MAX / GROWTH_FACTOR ||
        real_size > SIZE_MAX - request_size) {
  ...
  *base = ALLOC_SIZE * (pool->size) + pool->page_offset;
  ...
  do {
    ...
    if (pool->memory[pool->size] == NULL) {
      fprintf(stderr,
              "failed allocation of memory_pool_t in memory_pool_request\n");
      return false;
    }
