...
memory_pool_t* memory_pool_create(size_t initial_capacity) {
    ...
    pool->memory = (char**) malloc(sizeof(char*) * pool->capacity);
    ...
    pool->memory[0] = (char*) malloc(ALLOC_SIZE);
...
bool memory_pool_request(memory_pool_t* pool, const char * request, size_t request_size, size_t * memory_id, size_t * lower_bound) {
    ...
    while (copied < request_size) {
        ...
        pool->memory[pool->size] = (char*) malloc(ALLOC_SIZE);
...
char memory_pool_access(const memory_pool_t* pool, size_t memory_id, size_t offset) {
    size_t page_offset = offset % ALLOC_SIZE;
    size_t pages = offset / ALLOC_SIZE;

    memory_id += pages;
    if (memory_id >= pool->capacity) {
        fprintf(stderr, "memory_id out of bounds in memory_pool_access\n");
        return OUT_OF_BOUNDS;
    }

    return pool->memory[memory_id][page_offset];
}
...
void memory_pool_grow(memory_pool_t * pool, size_t new_capacity) {
    ...
    char ** new_memory = (char**) malloc(sizeof(char*) * pool->capacity);
